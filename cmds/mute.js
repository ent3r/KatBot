...
module.exports = {
    ...
    details: {
        perms: "Admin",
        command: "!mute @user/id OPTIONAL: duration(d,h,m,s default:m) reason",
        cmd: "mute",
    ...
    run: ({
        ...
        if (req.length == 0) {
            message.channel.send(`No tag/ID provided!\nUsage: \`!mute @user/id OPTIONAL: duration(d,h,m,s default:m) reason\``)
            ...
            try {
                ...
                pool.query(sql, (err, res, fields) => {
                    ...
                    pool.query(`SELECT p_id from logs where timestamp = ${timestamp}`, (err, res, fields) => {
                        sendEmbed(message.channel, `Muted ${m.displayName}. Case number ${res[0].p_id}`)
                    })
                })
            } catch (e) {
                console.error(e);
